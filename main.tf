...
resource "null_resource" "configure-happ" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "chmod +x *.sh",
      "URL=${var.hashi_products[random_shuffle.happ_name.result[0]]} HAPP=${random_shuffle.happ_name.result[0]} PREFIX=${data.terraform_remote_state.haarg.outputs.rg_name} FQDN=${data.terraform_remote_state.haan.outputs.happ_fqdn} VMNAME=${data.terraform_remote_state.haavm.outputs.vmname} WATFVER=${var.tfver} RGTFVER=${data.terraform_remote_state.haarg.outputs.tfver} NTFVER=${data.terraform_remote_state.haan.outputs.tfver} VMTFVER=${data.terraform_remote_state.haavm.outputs.tfver} ./deploy_app.sh",
    ]
